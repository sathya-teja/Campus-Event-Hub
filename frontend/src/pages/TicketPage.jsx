...
export default function TicketPage() {
  ...
  const [pdfLoading, setPdfLoading] = useState(false); // ✅ NEW
  const [shareLoading, setShareLoading] = useState(false); // ✅ NEW
  ...
  return (
    <div style={styles.page}>
      ...
      {/* ✅ ticketRef wraps only the visual ticket — not the action buttons */}
      <div ref={ticketRef} style={styles.card} className="ticket-card">

        {/* Event image banner */}
        {event.image && (
          <div style={styles.banner}>
            <img src={getImageUrl(event.image)} alt={event.title} style={styles.bannerImg} crossOrigin="anonymous" />
        ...
        <div style={styles.infoSection}>
          ...
          <div style={styles.qrSection}>
            <p style={styles.qrLabel}>
              {attended ? "Attendance verified" : "Show this to the admin at the entrance"}
            </p>

            {/* ✅ Attended stamp overlay */}
            <div style={{ position: "relative", display: "inline-block" }}>
              <div style={{ ...styles.qrWrapper, opacity: attended ? 0.35 : 1 }}>
                {qrDataUrl ? (
              ...
              {/* ✅ Big stamp overlay when attended */}
              {attended && (
                <div style={styles.stampOverlay} className="stamp-overlay">
                  <div style={styles.stampInner}>
                    ...
                    <span style={styles.stampText}>ATTENDED</span>
                  </div>
                </div>
              )}
            </div>

            {/* Attended timestamp pill */}
            {attended && attendedAt && (
      ...
      {/* ✅ Action buttons — outside ticketRef so they don't appear in PDF */}
      <div style={styles.actions}>
        ...
        >
          {shareLoading ? (
            <span style={{ ...styles.spinner, borderTopColor: "#1d4ed8" }} />
          ) : (
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
        ...
        {/* Refresh */}
        <button style={styles.btnIcon} onClick={() => load(true)} disabled={refreshing} title="Refresh">
          {refreshing ? (
            <span style={styles.spinner} />
          ) : (
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 11-2.12-9.36L23 10"/></svg>
          )}
        </button>
      </div>
    </div>
...
const styles = {
  ...
  stampInner: {
    ...
    boxShadow: "0 2px 16px rgba(22,163,74,0.2)",
  },
  stampText: {
    fontSize: 13,
    fontWeight: 800,
  ...
  codeSection:     { marginTop: 20, textAlign: "center" },
  codeDivider:     { display: "flex", alignItems: "center", gap: 8, marginBottom: 14 },
  codeDividerLine: { flex: 1, height: 1, background: "#e5e7eb" },
  codeDividerText: { fontSize: 11, color: "#9ca3af", fontWeight: 500, whiteSpace: "nowrap" },
  codeLabel: {
  ...
  codeHint: { fontSize: 11, color: "#9ca3af", marginTop: 4 },

  // ✅ Updated actions — now below the card, full width
  actions: {
    display: "flex",
    gap: 10,
    marginTop: 16,
...
const ticketStyles = `
  ...
  .stamp-overlay {
    animation: stamp-pop .4s cubic-bezier(.22,.68,0,1.4) forwards;
  }
  @keyframes stamp-pop {
